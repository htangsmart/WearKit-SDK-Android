...
import com.topstep.wearkit.sample.databinding.ActivityBloodPressureConfigBinding
...
class BloodPressureConfigActivity : BaseActivity(), TimePickerDialogFragment.Listener,
    SelectIntDialogFragment.Listener {

    private val wearKit = MyApplication.wearKit
    private lateinit var viewBind: ActivityBloodPressureConfigBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewBind = ActivityBloodPressureConfigBinding.inflate(layoutInflater)
        setContentView(viewBind.root)
        supportActionBar?.setTitle(R.string.ds_blood_pressure_config)
        observeDispose1 = wearKit.bloodPressureAbility.observeMonitorConfig(true)
        ...
        viewBind.itemMonitorConfig.getSwitchView().setOnCheckedChangeListener { buttonView, isChecked ->
            if (buttonView.isPressed) {
                setMonitor(
                    wearKit.bloodPressureAbility.getMonitorConfig().copy(isEnabled = isChecked)
                )
            }
        }
        viewBind.itemStart.setOnClickListener {
            TimePickerDialogFragment.newInstance(
                timeMinute = wearKit.bloodPressureAbility.getMonitorConfig().start,
                title = getString(R.string.ds_config_start_time)
            ).show(supportFragmentManager, DIALOG_START)
        }
        viewBind.itemEnd.setOnClickListener {
            TimePickerDialogFragment.newInstance(
                timeMinute = wearKit.bloodPressureAbility.getMonitorConfig().end,
        ...
        viewBind.itemInterval.setOnClickListener {
            val value = wearKit.bloodPressureAbility.getMonitorConfig().interval
        ...
        if (wearKit.getRawSDK() is PbSDK) {
            ...
            viewBind.itemPbTestGetConfig.setOnClickListener {
                getDispose = (wearKit.getRawSDK() as PbSDK).configGetTest.getBloodPressureConfig()
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe({
    ...
    private fun setMonitor(config: WKBloodPressureMonitorConfig) {
        setDispose?.dispose()
        setDispose = wearKit.bloodPressureAbility.setMonitorConfig(config)
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({
    ...
    override fun onDialogTimePicker(tag: String?, timeMinute: Int) {
        when (tag) {
            DIALOG_START -> {
                setMonitor(
                    wearKit.bloodPressureAbility.getMonitorConfig().copy(
                        start = timeMinute
                    )
                )
            }
