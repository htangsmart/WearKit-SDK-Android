...
public class ColorPickerView extends View {
    ...
    private void init(AttributeSet attrs) {
        // 获取自定义组件的属性
        TypedArray types = mContext.obtainStyledAttributes(attrs, R.styleable.color_picker_view);
        try {
            mBigCircle = types.getDimensionPixelOffset(R.styleable.color_picker_view_circle_radius, 100);
            mRudeRadius = types.getDimensionPixelOffset(R.styleable.color_picker_view_center_radius, 10);
            mCenterColor = types.getColor(R.styleable.color_picker_view_center_color, Color.WHITE);
        } finally {
            types.close(); // TypeArray用完需要recycle
        }
    ...
    private Bitmap createColorWheelBitmap(int width, int height) {
        ...
        SweepGradient sweepGradient = new SweepGradient(width / 2.0f, height / 2.0f, colors, null);
        RadialGradient radialGradient = new RadialGradient(width / 2.0f, height / 2.0f, mBigCircle, 0xFFFFFFFF, 0x00FFFFFF, Shader.TileMode.CLAMP);
        ComposeShader composeShader = new ComposeShader(sweepGradient, radialGradient, PorterDuff.Mode.SRC_OVER);
        mPaint.setShader(composeShader);
        Canvas canvas = new Canvas(bitmap);
        canvas.drawCircle(width / 2.0f, height / 2.0f, mBigCircle, mPaint);
        return bitmap;
    }
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        // 画中心小球
        canvas.drawCircle(mRockPosition.x, mRockPosition.y, mRudeRadius, mCenterPaint);
    }
